    ...
    var body: some View {
        ...
        LazyVGrid(columns: [GridItem(),GridItem(),GridItem()]){
            /*
             ...
             Step 1.47,Currency.allCase ,pongo esto para que itere todo.*/
            ForEach(Currency.allCases){ currency in
                //Step 53,con self le decimos que queremos el nombre de la variable ,no del currency del Foreach
                if self.currency == currency{
                    
                    //Step 1.50, accedemos a los valores de image y name.CurrencyIcon es una vista.
                    CurrencyIcon(currencyImage: currency.image, currencyName: currency.name)
                        //V-20,Step 1.51,agregamos modifiers
